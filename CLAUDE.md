...
**M1 (index core), M2 (query tools, Step 4), M3 (plugin and review manifest, Steps 5–6), M4 (gates and context pack, Step 7), M5 (delegation and the local-LLM adapter, Step 8) and M6 (telemetry, report and benchmark harness, Step 9) are implemented: every milestone in the Getting Started guide is built.** What remains is the pilot itself: the benchmark's real task suite, the pilot repos and the open questions below. The repo holds the Python core under `src/compass/`, the Claude Code plugin under `plugin/`, its tests, and the five Markdown specs for Compass. `.docx` and `.pdf` exports of the specs live one directory up; the `.md` files here are the source of truth, and the exports predate M1.
...
- **M6:** telemetry (`telemetry.py`, TM-01, TM-02): one row per turn and per subagent in `.compass/telemetry.jsonl`, read from Claude Code's transcripts by the Stop and SubagentStop hooks, with an opt-in export. `compass report` (`report.py`, TM-03) compares tasks with Compass on and off. The benchmark harness (`bench/run.py`, Evaluation Plan) runs tasks headless in both conditions and writes the plan's report through `compass report`. `query.enabled` completes CF-03, so a telemetry-only pilot baseline is possible. A weekly job on both CI systems runs every done-when check against the latest Claude Code (NF-14).

The M2 to M6 done-when checks run real headless Claude Code sessions, so they are scripts rather than pytest tests (see Testing). All passed on 2026-09-24 with Claude Code 2.1.281 and claude-opus-5-5. M2: "where is retry handled and what calls it?" was answered with `find_symbol`, `read_symbol` and `callers_of` and zero Read, Grep or Glob calls. M3: on the Python, TypeScript and Go fixtures the change got anchors and a manifest, the reply stayed under 12 lines, the commit with tags was refused, and it went through after `compass accept`. M4: block mode refused "improve the error handling" before the model ran; warn mode had Claude ask which code to focus on and change nothing; a refactor got a spec draft with four open questions and no code edits, then was implemented, tagged, after `compass approve`. M5: asked to run a suite whose `python3 -m unittest -v` output is 623 lines, Claude handed the run to `compass:test-runner` and got back an 8-line answer naming both failures (3 of 3 runs, once the rule said that `tail` output counts too; 1 of 2 before); with gemma3 on Ollama a commit took 0.20 s with `local_llm` on or off, and the summaries reached the map afterwards. M6: the delegation check's telemetry rows matched Claude Code's own per-model token counts exactly, for the main model and the Haiku subagent. The harness ran both example tasks with Compass on and off (4 sessions, all passing), and `compass report` wrote the on/off report. On that five-file fixture Compass used about 47% more main-model tokens: session-start context, a ToolSearch round trip (Claude Code 2.1 defers MCP tools) and a two-test run delegated to `test-runner`, with nothing to save in return. The real suite needs 50k+ LOC repos.
...
| `docs/setup-guide.md`, `docs/user-guide.md` | For users: installing Compass, working with it, and a ten-minute demo; keep their sample output real |
...
3. **State** (`.compass/` in each target repo) — `config.yaml` and `standards/` are committed; `index.db`, `map/`, `changes/`, `state.json` and `logs/` are derived, gitignored and rebuildable with `compass index --full`. `summaries.db` (local-model summaries) is gitignored too, but only `compass enrich` can rebuild it, so `--full` keeps it. `telemetry.jsonl` is gitignored and cannot be rebuilt at all: it is the measurement record.
...
- **Built on Claude Code, never on an API key.** Compass works through Claude Code and the developer's own Claude login: nothing in it calls the Anthropic API or asks for an API key. The e2e checks and the benchmark run `claude` as a logged-in user, and CI logs Claude Code in with a subscription token from `claude setup-token`. `tests/test_plugin.py` fails if code, plugin, benchmark, e2e scripts or CI mention `ANTHROPIC_API_KEY`.
...
compass report [PATHS] [--since D] [--json]  # tasks with Compass on vs off (TM-03); a bench/results folder gives the benchmark report
...
Still open: the optional `compass watch` (IX-15). Hooks all route through `compass hook <session-start|prompt|pre-edit|post-edit|stop|delegate|subagent-stop>`, all handled; any other event name passes through with exit 0.

The benchmark harness is `uv run python bench/run.py --model <id> [--tasks bench/tasks/<suite>] [--repeats 3]`, then `compass report bench/results/<batch>`; `bench/README.md` has the task format. Every run is a real headless session, so a batch spends Claude usage.
...
```bash
uv run pytest                     # unit, snapshot, determinism, hook-contract tests (~70 s, mostly subprocesses)
...
`tests/test_query.py` covers every tool per language; `tests/test_mcp.py` drives the real server over stdio with the MCP client, as Claude Code does. The done-when checks run real headless Claude Code sessions, so pytest never runs them. `uv run python tests/e2e/check_query_tools.py` (M2) passes only if Claude answers through Compass with zero Read calls. `uv run python tests/e2e/check_review_loop.py [--fixture python_app|ts_app|go_app]` (M3) loads the plugin with `--plugin-dir` and has Claude make a small change, then checks the manifest, the tags, the reply length, the refused commit and the commit after accept. `uv run python tests/e2e/check_gates.py` (M4) runs block mode, warn mode and a large task through its spec, approval and implementation (a resumed session). `uv run python tests/e2e/check_delegation.py [--local-model gemma3]` (M5) asks Claude to run a 600-test suite with two failures and passes only if the run went to a Compass subagent, never to the main conversation's Bash, and came back short; with `--local-model` it also commits with `local_llm` on and off, compares the times and waits for the summaries (it needs a model already running, for example `ollama serve`; `--skip-session` runs only that part, with no Claude usage). Each costs roughly 1–7k output tokens. `uv run python tests/e2e/run_all.py [--model sonnet]` runs all of them; the weekly CI job calls it. Neither can use `--bare`, which accepts only an API key and never a claude.ai login, so they keep other MCP servers out with `--strict-mcp-config` instead.

M6's tests are `tests/test_telemetry.py` (transcripts read into rows, against `tests/transcripts.py`, a builder for synthetic transcripts in Claude Code's line format, plus the Stop, SubagentStop and SessionStart hook contracts), `tests/test_report.py` (roll-up, the on/off comparison, the benchmark statistics and the decision rule) and `tests/test_bench.py` (the harness end to end with `tests/fake_claude.py`, a stand-in for `claude -p` that spends nothing). M5's tests are `tests/test_delegation.py` (the contracts, the SubagentStop, Agent and scaffold pre-edit hook contracts, binding parallel delegations, the SessionStart rules) and `tests/test_llm.py` (the client, the local tools, enrichment end to end and a real commit), both against `tests/fake_llm.py`, an OpenAI-compatible server on 127.0.0.1 that records every request; `tests/test_mcp.py` checks the local tools come and go with the model. M4's tests are `tests/test_gates.py`: prompt kinds, candidate names, labels, every rule, rule discovery, sizing, the pack against an indexed fixture, specs and approval, and the prompt and pre-edit hook contracts end to end. M3's tests: `tests/test_anchors.py` (scanner, stripper, staged-lines check), `tests/test_manifest.py` (grouping, links, hosted URLs for every GitHub and Azure DevOps remote form), `tests/test_review.py` (hook contracts for the whole review loop, `accept`, and real `git commit` runs against the pre-commit hook) and `tests/test_plugin.py` (plugin files against SchemaStore schemas vendored in `tests/schemas/`, hooks wired to handled events, versions in step, and `claude plugin validate --strict` when Claude Code is installed). Test files assemble tags at runtime (`AI = "@ai" + ":"`): a literal tag in Compass's own files would trip its own pre-commit check, and `test_compass_itself_carries_no_anchor_tags` enforces that.

Fixture repos live in `tests/fixtures/` (one per language family); `tests/snapshots/<fixture>/` holds the expected index rows, map shards, file list and stack profile. Tests isolate git from the developer's config and set `COMPASS_CTAGS=off`, so installed tools never change snapshot output. Latency checks use plain timers with p95 rather than `pytest-benchmark`. CI is macOS, Ubuntu and Windows on Python 3.11 and 3.12 — Windows without WSL is a requirement (NF-08), not an afterthought. It is defined twice, identically: `.github/workflows/ci.yml` (GitHub Actions) and `azure-pipelines.yml` (Azure Pipelines). Change both together; `tests/test_ci_configs.py` fails when their matrices or commands drift apart, and it also validates both files against their published schemas, so the Azure pipeline gets checked on every GitHub run. The weekly compatibility job (NF-14) is a second pair, `.github/workflows/e2e.yml` and `azure-pipelines-e2e.yml`, with the same schedule and commands. It installs the latest Claude Code and runs `tests/e2e/run_all.py` on one Linux leg. It logs Claude Code in with your Claude subscription, never an API key: run `claude setup-token` once and store the token as a `CLAUDE_CODE_OAUTH_TOKEN` secret. Without it the job does nothing. Runs count against the subscription's usage like any session; an `E2E_MODEL` variable picks a lighter model.
...
- Spend minutes carefully. Public repos are free; private repos get 2,000 minutes a month, and macOS costs about 10× Linux. The workflow cancels superseded runs, skips pushes that only touch root-level docs or `docs/`, and runs one macOS leg in private repos.
...
- **Telemetry** (`telemetry.py`):
  - The Stop hook reads the session transcript (`transcript_path`) from where the last Stop left off (byte offsets in `.compass/telemetry-offsets.json`, under `telemetry.lock`) and appends a `turn` row to `.compass/telemetry.jsonl`. SubagentStop does the same with `agent_transcript_path`, as a `subagent` row.
  - Rows carry the active task and its `Category:`/`Size:` tags (labels in the brief). They also record whether Compass counted as on (any of query, prompt_gate, context_pack, spec_gate, review, delegation) and which modules were on. They hold counts, timings and tags only, never prompts, code or paths; MCP tools other than Compass's collapse to `mcp`.
  - `telemetry.export: <file>` appends every row there as well (TM-02). `telemetry.enabled: false` stops recording.
  - The transcript format, as probed on Claude Code 2.1.281:
    - There is one line per content block. Each line of an API message repeats its usage, with output tokens still growing on the early lines, so each message id counts once, at its per-field maximum.
    - Tool results carry a `promptId` too. A typed prompt is a `user` line that is not `isMeta` (a Stop hook's block reason arrives as an `isMeta` "Stop hook feedback:" line), not `isCompactSummary`, not a tool result, and not "[Request interrupted…" or "<task-notification>".
    - `queue-operation` lines carry the time a prompt was typed, so they never count for timing.
    - Subagents write `<session>/subagents/agent-<id>.jsonl`, with a `.meta.json` naming the agent type. `--resume` appends to the same file.
  - Claude Code writes transcripts asynchronously, and a hook can start about 100 ms before the turn's last message is on disk. So the hooks wait, up to 1.5 s, until the last model message has a final `stop_reason` (neither none nor `tool_use`). Without that wait the first real check came up one message short.
  - Active time is the sum of gaps between lines, leaving out every gap that ends at a typed prompt (the developer's wait). A subagent's time falls inside its turn, so the report never adds it.
  - SessionStart with source `resume` or `compact` starts the session's offset after the existing history, so a resumed conversation is not counted twice.
  - Compass's own injected context is measured too: SessionStart's stdout from `compass hook`, UserPromptSubmit context starting `[compass]`, and Compass's feedback lines.
  - The e2e delegation check asserts that the rows equal Claude Code's own per-model counts, so a format change fails the weekly job.
- **Report** (`report.py`): rows roll up per task, keyed by file and task id, or by session for turns outside a task. A task whose rows disagree on the Compass flag counts as mixed and is left out.
  - For telemetry, it shows medians per task, off against on, overall and by category. For this repo's own telemetry it adds the prompt gate's bypass rate from `logs/gate.jsonl` (target: under 20% of prompts).
  - For benchmark folders, it takes each task's median over runs in each condition and computes Δ = (ON − OFF)/OFF. It then reports the median Δ across tasks with a seeded 10,000-resample bootstrap interval, the same for passing runs only, pass counts, categories, ablations against full ON, and the plan's go/narrow/stop rule for the benchmark's half. A batch smaller than 20 tasks × 3 runs is labelled a harness check.
- **Switches and the baseline**:
  - `query.enabled: false` removes the query tools and their SessionStart rule. The MCP server registers no tools, and calls to one already running answer that they are off.
  - The stack lines at SessionStart follow `context_pack.enabled`, and the pending-approval reminders follow `spec_gate.enabled`.
  - With every module off but telemetry, SessionStart prints nothing: that is the pilot's telemetry-only baseline.
- **Benchmark harness** (`bench/run.py`; `bench/README.md` has the task format):
  - Each run starts from a fresh copy of the task's repo: a fixture, or a clone from a mirror under `bench/.cache/`. For ON conditions, `compass init` runs first and is timed on its own.
  - Claude runs as `claude -p --session-id … --setting-sources project,local --strict-mcp-config --max-turns N`. `--max-turns` is missing from `claude --help` in 2.1.281 but still works.
  - When the spec gate holds a task, the harness runs `compass approve` and resumes the session.
  - After the session it copies the hidden files in and runs the acceptance check. `row.json` uses the telemetry row format plus a `bench` section: condition, pass, cost (Claude Code's list-price estimate from its result event; a subscription is not billed per token), wall time and index time.
  - The harness calls Compass as `python -m compass`; a PATH shim serves Claude Code's hooks, so it also runs on Windows.
...
- **Keep telemetry on** (the default, `telemetry.enabled`): baseline data from real sessions beats estimates, and the pilot's first weeks run with every other module off.
